...
class TrackService {
  ...
  async getTracks(params = {}) {
    console.log('Запрос треков с параметрами:', params);
    const response = await apiClient.get('/tracks', { params });
    console.log('Получено треков:', response.data.tracks?.length || 0);
  ...
  async deleteTrack(id) {
    console.log(`Запрос на удаление трека с ID: ${id}`);
    try {
      const response = await apiClient.delete(`/tracks/${id}`);
      console.log('Трек успешно удален');
      return response.data;
    } catch (error) {
      console.error('Ошибка при удалении трека:', error.response?.data || error.message);
      throw error;
    }
  }
